...
int main(void)
{
    ...
    while (1)
    {
        ...
        {
            ualarm(50000, 0);  // Установка таймера для генерации сигнала SIGALRM
            while (!isSignal)
            {
                usleep(9500 + rand() % 3000);  // Задержка перед генерацией пары значений
                if (pair.first == 0)
                {
                    pair.first = 1;  // Генерация случайного значения для первого бита
                    usleep(8000 + rand() % 3000);  // Задержка перед генерацией второго бита
                    pair.second = 1;  // Генерация случайного значения для второго бита
                }
                else
                {
                    pair.first = 0;  // Генерация случайного значения для первого бита
                    usleep(8000 + rand() % 3000);  // Задержка перед генерацией второго бита
                    pair.second = 0;  // Генерация случайного значения для второго бита
                }
            }
